...
class NotificationBanner(Directive):
    ...
    def run(self) -> List[nodes.Element]:
        ...
        # Create a heading containing the required argument of the directive, and compile this within a `<div>` tag
        notification_title = f"<h2 class=\"govuk-notification-banner__title\">{self.arguments[0]}</h2>"
        notification_header = nodes.raw("", notification_title, format="html",
                                        classes=["govuk-notification-banner__header"])

        # Render the content within a `<p>` tag, parsing the actual text
        notification_heading = nodes.paragraph(classes=["govuk-notification-banner__heading"])
        ...
        notification_content = nodes.container(classes=["govuk-notification-banner__content"])
