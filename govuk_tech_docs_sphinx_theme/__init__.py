...
JAVASCRIPT_FILES = sorted([j.relative_to(DIR_STATIC).as_posix() for j in DIR_JAVASCRIPTS.rglob("*.js")
...
def setup(app) -> None:
    ...
    app.add_html_theme("govuk_tech_docs_sphinx_theme", Path.resolve(Path(__file__).parent))
